...
const checkFirstName = body('firstName')
    .trim().bail()
    .isLength({ min: 1 }).withMessage('Имя обязательно для заполнения')
    .custom(firstName => {
...
const checkLastName =  body('lastName')
    .trim().bail()
    .isLength({ min: 1 }).withMessage('Фамилия обязательна для заполнения')
    .custom(lastName => {
...
const checkAge =  body('age')
    .trim()
    .isInt({ min: 1 }).withMessage('Возраст должен быть положительным целым числом');
